            ...
            @foreach ($sessionData as $item)
                ...
                @foreach ($dayWeek as $day)
                    @foreach (['MatinSE1', 'MatinSE2', 'AmidiSE3', 'AmidiSE4'] as $sessionType)
                        ...
                        @foreach ($sissions as $sission)
                            @if ($sission->day === $day &&
                                ...
                                style="background-color:{{ $isActive ? 'rgba(12, 72, 166, 0.3)' :  ''}} ; height: 50px !important; overflow: hidden"
                                 ...
                                 colspan="1" rowspan="1" data-bs-toggle="modal" data-bs-target="#exampleModal" class="TableCases" id="{{ $day . $sessionType . $group->id }}">
                                    @if ($item === 'Formateur')
                                        {{ $sission->user_name }}
                                    @elseif ($item === 'Module')
                        ...
                        @endforeach
                        @if (!$sessionFound)
                            <td wire:click="updateCaseStatus(true , true)" style="height: 50px !important; overflow: hidden" colspan="1" rowspan="1" data-bs-toggle="modal" data-bs-target="#exampleModal" class="TableCases" id="{{ $day . $sessionType . $group->id }}"></td>
                        @endif
                    @endforeach
                @endforeach
            </tr><tr>
